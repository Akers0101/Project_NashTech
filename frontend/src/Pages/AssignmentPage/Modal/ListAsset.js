...
const [AssetResponse,setAssetResponse] = useState({
  ...
  const { loading,gridData2, openPopup, setOpenPopup, selectedValue, assignment } = props;
 
  
 
 
const modifiedData = gridData2
.filter((item) => {
 return( 
     item.assetState === "Available" &&
...
.map(({ ...item }) => ({
  ...
  const handleShow = () => {
    setOpenPopup(false);
    selectedValue({
      assetId: AssetResponse.toAssetId,
      assignedByUserId:userId,
      assignedToUserName: assignment.assignedToUserName,
      assignedToUserId: assignment.assignedToUserId,
      assetName: AssetResponse.AssetName,
  ...
  const columns = [
    {
      ...
      renderCell: (item) => {
        return (
          <Radio 
          key={item.assetId}
          checked={AssetResponse.toAssetId === item.id} 
